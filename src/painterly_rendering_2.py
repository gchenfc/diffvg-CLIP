"""
NOTE: THIS IS ALMOST UNCHANGED FROM THE SOURCE HERE:
...
def main(args):
    ...
    call(['rm', '-r', 'results/painterly_rendering'])
        ...
        for i in range(num_paths):
            ...
            path_group = pydiffvg.ShapeGroup(shape_ids = torch.tensor([len(shapes) - 1]),
                                             ...
                                             stroke_color = torch.tensor([random.random(),
                                                                          ...
                                                                          random.random()]))
            if i > 8:
                path_group.stroke_color = shape_groups[i % 8].stroke_color
            shape_groups.append(path_group)
    ...
    with open('results/painterly_rendering/args.txt', 'w') as f:
        f.write(str(args))

    import time
    fnamebase = args.target.split('/')[-1].split('.')[0]
    foldername = time.strftime('%Y-%m-%d-%T')
    call(['cp', '-r', 'results/painterly_rendering', 'results2/{:}_{:}'.format(fnamebase, foldername)])
    print('results2/{:}'.format(fnamebase))
